                ...
                <table class="table table-hover">
                    ...
                    <tbody id="load">
                        <c:forEach items="${pageInfo.list}" var="scenicVo" varStatus="id">
                            <tr>
                                <td>
                                    <input type="checkbox" class="input-control" name="checkbox[]" value="${scenicVo.scenicid}" />
                                </td>
                                <td class="article-title">${scenicVo.scenicid}</td>
                                <td>${scenicVo.name}</td>
                                <td>
                                    <c:if test="${scenicVo.stage eq 0}">
                              ...
                              <!--   <td>${scenicVo.tel}</td>  
                                ...
                                <button type="button" class="btn btn-info" data-toggle="modal"
                                        data-target="#publishScenic" name="publish">发布
                                </button>   
                                <button type="button" class="btn btn-info" data-toggle="modal"
                                        data-target="#myModal" name="edit">修改
                                </button> 
                                <a href="${pageContext.request.contextPath}/backScenic/downScenic?id=${scenicVo.scenicid}">
                                    <button type="button" class="btn btn-warning">下架</button>
                                </a> 
                                <a href="${pageContext.request.contextPath}/backScenic/upScenic?id=${scenicVo.scenicid}">
...
<div class="modal fade" id="publishScenic" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <script>  
        //取页面的值传给模态框
        $(function() {
            $("Button[name='publish']").each(function() {
                $(this).click(function() {
                	debugger;
                   var contentid = $(this).parent().parent().children().eq(1).text();
                   var title = $(this).parent().parent().children().eq(2).text();
                    
                   var areanum = $(this).parent().parent().children().eq(10).text();

                   var pic = $(this).parent().parent().children().eq(14).text();
                
                   var stage = $(this).parent().parent().children().eq(17).text();
                   var path = $(this).parent().parent().children().eq(18).text();
