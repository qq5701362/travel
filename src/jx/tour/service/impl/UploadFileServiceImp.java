...
public class UploadFileServiceImp implements UploadFileService {
        ...
        if (files != null && files.length > 0) {
            ...
            for (MultipartFile file : files) { //循环遍历，取出单个文件，下面的操作和单个文件就一样了
                if (!file.isEmpty()) {//这个判断必须加上  
                    ...
                    //新文件名
                    String newFileName = UUID.randomUUID() + originalFileName.substring(originalFileName.lastIndexOf("."));
                    String save ="/area/scenic/"+newFileName;
                    picPath.add(save);  
                    //获得物理路径webapp所在路径，即新文件将要保存的路径
                    //String pathRoot = "E:/upload/temp/"+save;
                    String pathRoot = origin + save;
                    //新图片
        ...
        if (files != null && files.length > 0) {
            ...
            for (MultipartFile file : files) { //循环遍历，取出单个文件，下面的操作和单个文件就一样了
                if (!file.isEmpty()) {//这个判断必须加上  
                    ...
                    //新文件名
                    String newFileName = UUID.randomUUID() + originalFileName.substring(originalFileName.lastIndexOf("."));
                    String save = "/area/specialty/"+newFileName;
                    picPath.add(save);  
                    //获得物理路径webapp所在路径，即新文件将要保存的路径
                    //String pathRoot = "E:/upload/temp/"+save;
                    String pathRoot = origin + save;
        ...
        if (files != null && files.length > 0) {
            ...
            for (MultipartFile file : files) { //循环遍历，取出单个文件，下面的操作和单个文件就一样了
                if (!file.isEmpty()) {//这个判断必须加上  
                    ...
                    //新文件名
                    String newFileName = UUID.randomUUID() + originalFileName.substring(originalFileName.lastIndexOf("."));
                    String save = "/area/hotel/"+newFileName;
                    picPath.add(save);  
                    //获得物理路径,即新文件将要保存的路径
                    //String pathRoot = "E:/upload/temp/"+save;
                    String pathRoot = origin + save;
	...
	public String uploadContentImag(MultipartFile file,Content content) throws Exception {
        ...
        String newFileName = UUID.randomUUID() + originalFileName.substring(originalFileName.lastIndexOf("."));
        if(categoryid==0){
        	save = "/area/scenic/"+newFileName;
        }
        else{
        	save = "/area/specialty/"+newFileName;
        }
        //获得物理路径,即新文件将要保存的路径
        //String pathRoot = "E:/upload/temp/"+save;
        String pathRoot = origin + save;
